...
const Shape = ({ bgColor, borderColor }: PropertyColors) => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={418} height={298} fill="none">
...
const ProjectDetails = ({ name, description, techs, image, color, colorClass, href }: ProjectDetailsProps) => {
  const [filteredTechs, setFilteredTechs] = useState<{ label: string; icon: StaticImageData }[]>([]);

  const renderTechs = () => {
    const filteredTechsLocal = SKILLS.filter((skill) => techs?.includes(skill.label));
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[420px] max-lg:max-w-xl w-full cursor-pointer group" onClick={() => window.open(href, '_blank')}>
      ...
      <div className={twMerge('w-full h-[298px] rounded-lg border justify-center hidden max-lg:flex', colorClass.borderColor, colorClass.bgColor)}>
        <Image src={image} alt="" />
      </div>
